var dostoy = function () {
            ...
            if (evt.keyCode >= 48) {
                ...
                switch (evt.keyCode) {
                    case 8: // backspace
                        ...
                        break;
                    case 13: //enter
                        document.getElementById("mobileKeyboard");
    ...
    var startx = function (display) {
        document.open();
        document.write("<script> const go = new Go(); WebAssembly.instantiateStreaming(fetch('"+dostoy.getPath()+"/"+display+"'), go.importObject).then((result) => { go.run(result.instance); });</script>");   
        document.close();
        eval("window.addEventListener('keydown', function (event) { if(event.keyCode==27){ if (confirm('Esta seguro de salir?') == true) { dostoy.setCookiePath('"+dostoy.getPath()+"'); window.location='/';  } } }); ");
    }
    ...
    var setCookiePath = function (newPath) {
        const d = new Date();
        d.setTime(d.getTime() + (1 * 24 * 60 * 60 * 1000));
        let expires = "expires="+d.toUTCString();
        document.cookie = "ruta=" + newPath + ";" + expires + ";path=/";
    }
    ...
    return {
        ...
        getXPosition: function(){return curX;},
        getYPosition: function(){return curY;}
    }
