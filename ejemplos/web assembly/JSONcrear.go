...
func main() {
	...
	fmt.Printf("JSON vacio: %s\n", jsonVacio.Value)

	// 2. Crear un objeto JSON con datos básicos de persona
	persona := json.CreateEmptyJSON()
	persona = json.AddStringToJSON(persona.Value, "nombre", "Juan Perez")
	...
	pasatiempos = json.AddItemToArray(pasatiempos.Value, `"futbol"`)
	pasatiempos = json.AddItemToArray(pasatiempos.Value, `"lectura"`)
	pasatiempos = json.AddItemToArray(pasatiempos.Value, `"programacion"`)
	...
	infoLaboral = json.AddStringToJSON(infoLaboral.Value, "empresa", "Soluciones Tecnologicas")
	...
	fmt.Printf("\nPersona con informacion laboral:\n%s\n", persona.Value)

	// 8. Verificar si el JSON es válido
	esValido := json.IsValidJSON(persona.Value)
	fmt.Printf("\nJSON valido? %s\n", map[bool]string{true: "Si", false: "No"}[esValido])
}